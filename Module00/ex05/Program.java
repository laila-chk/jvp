...
public class Program {
  ...
  public static void assignKeys(String[][][] att, int totStd, int[] days, String[] timeTb) {
    //sort time table : |4 MO | 3 WE | 2 MO | ==> |3 WE |2 MO | 4 MO |     
    timeTb = sortTimeTb(timeTb, days);
    for (String s : timeTb)
      System.out.println("-" + s);
    //hard coding keys: |3 WE |2 MO | 4 MO |  ==> |3:00 2 |2:00 7 | 4:00 7 | 3:00 9|    
    //att[std][date/key][record]
    String[] allClasses = new String[getTotalClasses(days)];
    for(int i = 0; i < 30; i++){
      //while we're on the same day, inseert same date to allClasses, we'll change the loop cuz we have to loop over allClasses < totCLs
      //and keep incrementing until we find a day that we have in timeTb
      timeTb[i % timeTb.length]
    }
  ...
  public static void main(String[] args) {
    ...
    int totCls = getTotalClasses(days);
    String[][][] attendance = new String[totStd][totCls][2];

    assignKeys(attendance, totStd, days, timeTb);

    // Reading lines : Mike 2 28 NOT_HERE ...
    // put them in attendance[std][class] ?
  }
}
